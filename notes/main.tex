...
\begin{itemize}
	...
	\item \emph{In-context learning (ICL)} is a problem setup in natural language understanding where a large pre-trained language model observes a test instance and a few training examples as its input, and directly decodes the output without any update to its parameters.
...
\textcolor{magenta}{TODO} Add references and examples for in-context learning
...
ChatGPT is a conversational model based on GPT-3.5, a large-scale language model that can generate coherent and diverse texts on various topics.
